...
| 항목 | 내용 |
| --- | --- |
| 과정 | `codyssey-b1-2-linux-troubleshooting` |
| 분야 | AI/SW 기초 |
| 구분 | Linux와 OS |
| 학습 시간 | 40시간 |
| 문제 기술 | 기술적 설명 |
...
> 이 세 가지 중 하나가 실서버에서 발생하면 어떻게 대응해야 할까요?

로그를 남기지 않고 재부팅부터 하면 원인이 묻히고 같은 장애가 반복됩니다. 이번 미션에서는 관제 데이터를 근거로 원인을 추론하고, 분석 결과를 **GitHub Issue 형식의 기술 리포트**로 작성합니다.

개발자가 작성한 코드는 운영체제 위에서 프로세스 형태로 실행됩니다. 이 미션에서는 빌드된 프로그램을 운영 환경에서 실행하며 발생하는 다음 시스템 장애를 분석합니다.

- Memory Leak / OOM
- CPU Spike
- Deadlock

단순히 “프로그램이 종료되었다”라는 결과만 확인하는 것이 아니라, 관제 데이터와 로그를 통해 장애 원인을 추론해야 합니다. 이를 바탕으로 현업 개발자처럼 기술 리포트를 작성하며 **트러블슈팅 및 협업 커뮤니케이션 역량**을 기르는 것이 최종 목표입니다.

## 2. 최종 결과물

다음 산출물을 **PDF 또는 GitHub Repository 링크** 형태로 제출해야 합니다.

### 시스템 장애 분석 및 이슈 리포트 3건

아래 장애 유형별로 GitHub Issue 형식의 기술 보고서를 각각 작성합니다.

1. OOM Crash
2. CPU Latency
3. Deadlock

각 리포트에는 다음 항목이 모두 포함되어야 합니다.

- **발생 현상:** 장애가 어떻게 관측되었는지 서술
- **재현 경로 및 증거:** 로그, 명령어 출력, 스크린샷 등 객관적 증거 첨부
- **근본 원인:** 장애의 기술적 원인 분석
- **조치 내용:** 환경변수 조정 등 임시 조치와 그 결과
- **결과 확인:** 조치 전후의 Before & After 비교

### 이슈 리포트 Markdown 템플릿

각 리포트는 아래 구조를 따르되, 세부 내용은 자유롭게 작성할 수 있습니다.
...
- [ ] CPU 사용률 급상승 구간: `top`, `ps`, 관제 결과 캡처
...
미션을 완료한 학습자는 다음 내용을 스스로 설명할 수 있어야 합니다.

- 메모리 구조를 이해하고, 메모리 누수가 시스템 전체에 미치는 영향을 설명할 수 있다.
- 특정 프로세스의 CPU 과점유가 시스템 지연을 유발하는 원리를 설명할 수 있다.
- 자원 경쟁으로 발생하는 교착상태(Deadlock)의 개념을 이해하고, 프로세스가 멈춘 상태를 시스템 도구로 식별하여 진단할 수 있다.
...
제공 애플리케이션인 `agent-leak-app`을 실행하려면 아래 조건이 모두 충족되어야 합니다. 조건을 충족하지 않으면 부트 시퀀스에서 자동으로 실패 처리됩니다.
...
| `AGENT_LOG_DIR` | 로그 디렉터리 존재 및 쓰기 권한 필요 |
| `MEMORY_LIMIT` | 정수, `50~512` 범위 (단위: MB) |
| `CPU_MAX_OCCUPY` | 정수, `10~100` 범위 (단위: %) |
| `MULTI_THREAD_ENABLE` | `true/false` (`1/0`, `yes/no` 허용) |
| `secret.key` 파일 | `$AGENT_HOME/api_keys/secret.key` 존재, 내용은 `agent_api_key_test` |
| 네트워크 | `0.0.0.0:15034` 바인딩 가능 |

### 4.2 메모리 누수 원인 규명 및 리포팅

1. `monitor.sh`를 활용하여 대상 프로세스(`agent-leak-app`)의 물리 메모리 사용량이 시간 경과에 따라 증가하는 패턴을 관측합니다.
2. 프로세스가 예고 없이 중단되었을 때 실행 로그를 분석합니다.
3. 메모리 임계치 초과로 애플리케이션의 메모리 보호 정책(`MemoryGuard`)에 따라 강제 종료되었음을 나타내는 핵심 로그를 식별합니다.
4. 환경변수 `MEMORY_LIMIT`를 조정하여 프로그램이 더 오래 생존하는지 확인합니다.
5. 결과를 리포트에 Before & After로 기록합니다.

### 4.3 CPU 과점유 분석 및 리포팅

1. 관제 도구와 로그를 통해 시스템 전체 부하가 아닌 특정 프로세스(`agent-leak-app`)의 CPU 사용률이 급격히 상승하는 구간을 식별합니다.
2. 프로그램 실행 로그를 분석하여 해당 종료가 오류가 아니라 과점유 방지 정책(`Watchdog`)에 따른 시스템 보호 조치였음을 입증합니다.
3. 환경변수 `CPU_MAX_OCCUPY`를 조정하여 프로세스 종료 여부 또는 생존 시간 변화를 확인합니다.
4. 결과를 리포트에 Before & After로 기록합니다.

### 4.4 교착상태(Deadlock) 진단 및 리포팅

1. 프로세스가 종료되지 않고 살아 있으나(PID 존재), CPU/메모리 변화가 없고 로그 기록도 멈춘 무응답 상태임을 식별합니다.
2. 프로그램 로그의 마지막 기록을 분석하여 서로 다른 스레드가 상대방의 자원을 무한히 기다리는 상태임을 논리적으로 증명합니다.
3. 환경변수 `MULTI_THREAD_ENABLE`을 조정하여 데드락 재현/회피 결과를 비교합니다.
4. 비교 결과를 리포트에 기록합니다.

> [!TIP]
> Deadlock 개념이 생소하다면 아래 키워드를 먼저 학습한 후 미션을 진행하는 것을 권장합니다.
>
...
1. **프로그램 로그 데이터 분석**
   - 로그의 타임스탬프를 기반으로 프로세스 간 실행 순서와 교체 주기를 패턴화합니다.
2. **알고리즘 역추론**
   - 도출된 패턴을 근거로 현재 프로그램에 적용된 스케줄링 기법이 Round-Robin, FCFS, Priority 중 무엇인지 논리적으로 추론합니다.
3. **장단점 및 적합한 아키텍처 분석**
   - 추론한 스케줄링 알고리즘의 기술적 장단점을 서술합니다.
   - 어떤 성격의 서비스에 적합한지 분석합니다.
     - 예: 실시간 응답이 중요한 웹 서버
     - 예: 처리량이 중요한 배치 서버
...
- **우선순위 아님:** 특정 스레드가 자원을 독점하거나 긴급하게 처리되는 경향 없이 A, B, C가 공평하게 나누어 가지는 모습을 보입니다.
- **최종 결론:** 각 스레드가 정해진 시간 할당량만큼 CPU를 사용하고 자원을 반납하는 라운드 로빈 알고리즘으로 추론됩니다.

## 6. 개발 환경

- 제공된 바이너리(Python 기반)를 실행할 수 있는 Linux 환경
...
다음 Linux 표준 명령어 및 라이브러리를 사용합니다.
...
> [!NOTE]
> 아래 내용은 정답이 아닌 참고 예시입니다. 실제 문구와 디자인은 달라도 됩니다.
...
`agent-leak-app` 애플리케이션을 실행하고 약 10분이 경과하면 터미널에 `SELF-TERMINATED` 메시지가 출력되며 프로세스가 예고 없이 종료됩니다. 애플리케이션 내부의 메모리 보호 정책(`MemoryGuard`)에 의해 프로세스가 강제 종료되는 현상이 반복됩니다.
...
- **현상 분석:** 애플리케이션 로직 내부에서 생성한 데이터를 힙(Heap) 메모리에서 해제하지 않고 지속적으로 쌓는 메모리 누수(Memory Leak) 결함이 있는 것으로 판단됩니다.
- **시스템 동작:** 물리 메모리 사용량이 `MEMORY_LIMIT`에 도달하자, 애플리케이션 내부의 **MemoryGuard 정책**이 시스템 전체의 불안정을 방지하기 위해 해당 프로세스를 `SIGKILL`로 강제 종료했습니다.

##### 4. Workaround & Verification (조치 및 검증)

- **조치 내용:** `.bash_profile`의 환경변수 `MEMORY_LIMIT` 값을 기존 256MB에서 512MB로 상향 조정하여 임시로 가용 메모리를 확보했습니다.
- **검증 결과:** 설정 변경 후 재실행한 결과, 기존 종료 시점인 10분을 넘어 30분 이상 프로세스가 생존함을 확인했습니다.
- **추가 제안:** 근본적인 해결을 위해 소스 코드에서 불필요한 데이터를 주기적으로 삭제(`del` 또는 `pop`)하는 리팩터링이 필요합니다.
...
- `monitor.sh` 관제 로그의 CPU 사용률 변화 추이 (캡처/수치 첨부)
- 프로그램 실행 로그의 Watchdog 관련 로그 발췌
...
`agent-leak-app` 실행 후 프로세스가 종료되지 않고 PID가 유지되지만, CPU/메모리 변화가 없고 로그 출력도 완전히 멈춘 무응답 상태가 지속됩니다.

##### 2. Evidence & Logs (증거 자료)

- `ps -ef | grep agent` 결과: PID 존재 확인
- `top -H` 또는 `ps -L` 결과: 스레드별 CPU/MEM 변화 없음 확인
- 프로그램 실행 로그의 마지막 기록 발췌: `WAITING` 또는 `BLOCKED` 로그
...
| 구분 | 내용 |
| --- | --- |
| 데이터 파일 | `agent-app-leak.zip` |
| 단위 문제 | PDF 파일 |
| Intel 칩용 바이너리 | `agent-app-leak-x86` |
| Apple 칩용 바이너리 | `agent-app-leak-arm64` |
